...
DIR := SourceFile Token Parser SyntaxAnalyzer Interpreter
...
DEST := build/main_app
# Build Directories
BUILD_DIR := build build/main build/source $(addprefix build/source/,$(SRC_DIR)) $(addprefix build/deps/,$(SRC_DIR)) build/lib build/shared $(addprefix build/shared/,$(SRC_DIR))
# .o files
OBJ := $(patsubst %.cpp,build/source/%.o,$(SRC))
# Include folders
